...
public class ConstrainedTaskPlacer implements TaskPlacer {
  ...
  ConstrainedTaskPlacer(String requestId, double probeRatio){
    ...
    //[[FLORIN
    LOG.info("Creating ConstrainedTaskPlacer for requestId: "+requestId+" probeRatio: "+probeRatio);
  }


  public Map<InetSocketAddress, TEnqueueTaskReservationsRequest>
      getEnqueueTaskReservationsRequests_wrapper(
          TSchedulingRequest schedulingRequest, String requestId,
          Collection<InetSocketAddress> nodes, THostPort schedulerAddress){
	  LOG.info("ConstrainedTaskPlacer calling old enqueue task reservations request, probeRatio: "+probeRatio);
  ...
  getEnqueueTaskReservationsRequests(
        ...
        if (numEnqueuedNodes < probeRatio) {
          ...
          if (!requests.containsKey(addr)) {
            TEnqueueTaskReservationsRequest request = new TEnqueueTaskReservationsRequest(
                schedulingRequest.getApp(), schedulingRequest.getUser(), schedulingRequest.shortJob, requestId,
                schedulerAddress, hostPort, 1);
    ...
    if (unconstrainedTasks.size() > 0) {
      addRequestsForUnconstrainedTasks(
          unconstrainedTasks, requestId, schedulingRequest.getApp(), schedulingRequest.shortJob, schedulingRequest.getUser(),
  ...
  private void addRequestsForUnconstrainedTasks(
      List<TTaskSpec> unconstrainedTasks, String requestId, String appId, Boolean shortTask, TUserGroupInfo user,
    ...
    for (InetSocketAddress nodeMonitor : unusedNodeMonitors) {
      ...
      TEnqueueTaskReservationsRequest request = new TEnqueueTaskReservationsRequest(
          appId, user, shortTask, requestId, schedulerAddress, new THostPort(nodeMonitor.getAddress().getHostAddress(),nodeMonitor.getPort()), 1);
  ...
  public List<TTaskLaunchSpec> assignTask(THostPort nodeMonitorAddress, THostPort originalNodeMonitorAddress) {
    ...
    if (numOutstandingReservations == 1) {
      ...
      // TODO add case for originalNodeMonitorAddress!!!
      /* } else if (){
    */	
    } else {
...
@Override
public Map<InetSocketAddress, TEnqueueTaskReservationsRequest> getRelocateEnqueueTaskRequests(String requestId, List<InetSocketAddress> probestoRelocate,
		Collection<InetSocketAddress> nodes, THostPort schedulerAddress, Boolean roundsLeft) {
	// TODO Auto-generated method stub
	return null;
}

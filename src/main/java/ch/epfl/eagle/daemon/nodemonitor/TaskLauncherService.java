  ...
  private class TaskLaunchRunnable implements Runnable {
    ...
    public void run() {
      while (true) {
        ...
        LOG.info("Received task for request " + task.requestId + ", task " +
        ...
        LOG.info("Launched task " + task.taskSpec.getTaskId() + " for request " + task.requestId +
    ...
    private List<TTaskLaunchSpec> executeGetTaskRpc(TaskSpec task) {
      ...
      try {
        taskLaunchSpecs = getTaskClient.getTask(task.requestId, nodeMonitorInternalAddress, task.originalNodeMonitorAddress);
      } catch (TException e) {

...
/**
 * This class extends the thrift Eagle node monitor interface. It wraps the
...
public class NodeMonitorThrift implements NodeMonitorService.Iface,
  ...
  // The socket addr (ip:port) where we listen for requests from other Eagle daemons.
  ...
  public void initialize(Configuration conf, int nmPort, int internalPort)
    ...
    int threads = conf.getInt(EagleConf.NM_THRIFT_THREADS,
    ...
    int internalThreads = conf.getInt(
        EagleConf.INTERNAL_THRIFT_THREADS,
  ...
  public boolean enqueueTasksCentralized(TEnqueueTaskReservationsRequest request, List<String> notExecutingLong, double longStatusTimestamp)
      throws TException {
    return nodeMonitor.enqueueTasksCentralized(request, notExecutingLong, longStatusTimestamp);
  }
   ...
   **/  
  @Override
  public List<TEnqueueTaskReservationsRequest> sendTasksReservations() throws TException {
	return nodeMonitor.sendTaskReservations();
  }

  // EAGLE distributed schedulers get bitmap
	@Override
	public TEnqueueTaskReservationsResponse probeEnqueueTaskReservations(TEnqueueTaskReservationsRequest request) throws TException {
		return nodeMonitor.probeEnqueueTaskReservations(request);
	}
	
	// Gosipping
	@Override
	public void receiveGossip(List<String> notExecutingLong, double longStatusTimestamp, int round) throws TException {
		nodeMonitor.receiveGossip(notExecutingLong,longStatusTimestamp,round);
	}

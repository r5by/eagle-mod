...
namespace java ch.epfl.eagle.thrift

# SchedulerService is used by application frontends to communicate with Eagle
# and place jobs.
...
service GetTaskService {
  ...
  # HAWK adding info of old node monitor
  list<types.TTaskLaunchSpec> getTask(1: string requestId, 2: types.THostPort nodeMonitorAddress, 3: types.THostPort oldNodeMonitorAddress);
}

# A service used by application backends to coordinate with Eagle.
...
# The InternalService exposes state about application backends to other Eagle daemons.
service InternalService {
  ...
  # Hawk stealing
  list<types.TEnqueueTaskReservationsRequest> sendTasksReservations();

  # EAGLE 
  types.TEnqueueTaskReservationsResponse probeEnqueueTaskReservations(1: types.TEnqueueTaskReservationsRequest request);
  bool enqueueTasksCentralized(1: types.TEnqueueTaskReservationsRequest request, 2: list<string> notExecutingLong, 3: double longStatusTimestamp);
  void receiveGossip(1: list<string> notExecutingLong, 2: double longStatusTimestamp, 3: i32 round);
}

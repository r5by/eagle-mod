...
// A fully-specified Eagle task has four identifiers
// neeed?
struct TFullTaskId {
...
struct TSchedulingRequest {
  ...
  3: bool shortJob; 
  4: double estimatedDuration;
  5: TUserGroupInfo user;
  # A description that will be logged alongside the requestId that Eagle assigns.
  6: optional string description;
  # Hack to allow us to specify the probe ratio for certain types of requests.
  7: optional double probeRatio;
}
...
struct TEnqueueTaskReservationsRequest {
  ...
  6: THostPort originalNodeMonitorAddress;
  7: i32 numTasks;
}

# EAGLE
struct TEnqueueTaskReservationsResponse {
  1: bool socketOK;
  2: bool longInQueue;
  3: list<string> notExecutingLong;
  4: double longStatusTimestamp;
}
...
struct TTaskLaunchSpec {
  ...
  # Description of the task passed on to the application backend (opaque to Eagle).
  2: binary message;
}
...
# Represents the State Store's view of resource consumption on an Eagle node.
# TODO: will include information about per-user accounting.
struct TNodeState {
  1: TResourceVector eagleUsage;   # Resources used by Eagle
